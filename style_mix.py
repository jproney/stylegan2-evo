...
with torch.no_grad():
    ...
    for i in range(8):
        ...
        im[0, p1:p2] = w2[0, p1:p2] #im[0, p1:p2] + diff[0, p1:p2]  # shift some subinterval
        im = g_ema([im], input_is_latent=True, truncation=1)[0].cpu().numpy().squeeze()
        im = np.moveaxis(im, [0, 1, 2], [2, 0, 1])
        axs[i // 3, i - 3*(i//3)].imshow(im*.5 + .5)
    plt.show()
